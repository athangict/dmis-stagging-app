...
{
	protected $table = 'aca_result'; //tablename
 ...
 * @return Array
 */
public function getDistinctyear($data)
	{
		$adapter = $this->adapter;
	    $sql = new Sql($adapter);
	    $select = $sql->select();
	    $select->from($this->table)
		       ->columns(array(new Expression('DISTINCT(academic_year) as year')));
	    $selectString = $sql->getSqlStringForSqlObject($select);
		$results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
		return $results;
	}
	 ...
	 * @return int
	 */		
	public function getCountNC($data)
	{
		$adapter = $this->adapter;  	 
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from($this->table)
		       ->columns(array('count' => new Expression('COUNT(*)')))
			   ->where(array("organization" => $data['organization']))
			   ->where(array("academic_year" => $data['academic_year']));
		$selectString = $sql->getSqlStringForSqlObject($select);
	 ...
	 * @return int
	 */		
	public function getPasscount($data)
	{
		$adapter = $this->adapter;  	 
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from($this->table)
		       ->columns(array('count' => new Expression('COUNT(*)')))
			   ->where(array("class" => $data['class']))
			   ->where(array("pass" => $data['pass']))
			   ->where(array("organization" => $data['organization']))
			   ->where(array("academic_year" => $data['academic_year']));
		$selectString = $sql->getSqlStringForSqlObject($select);
	    //echo $selectString; exit;
	 ...
	 * @return int
	 */		
	public function getPasscountNL($data)
	{
		$adapter = $this->adapter;  	 
		$sql = new Sql($adapter);
		$select = $sql->select();
	 ...
	 * @return int
	 */		
	public function getTotalcountNL($data)
	{
		$adapter = $this->adapter;  	 
		$sql = new Sql($adapter);
		$select = $sql->select();
