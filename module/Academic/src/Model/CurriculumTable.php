...
class CurriculumTable extends AbstractTableGateway 
{
	protected $table = 'aca_curriculum_details'; //tablename
	...
	*/
	 ...
	 * @return Array
	 */
	public function getRegisteredAcademic($data,$where = NULL)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
	    $select = $sql->select();
		...
		if($data['academic_year'] != '-1'){
			$select->where(array('academic_year'=>$data['academic_year']));
		}
		if($data['organization'] != '-1'){
			$select->where(array('organization'=>$data['organization']));
		}
	 ...
	 * @return Array
	 */
	public function getExamReport($data,$where = NULL)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
	    $select = $sql->select();
		$select->from($this->table);
		if($data['batch'] != '-1'){
			$select->where(array('batch'=>$data['batch']));
		}
		if($data['organization'] != '-1'){
			$select->where(array('organization'=>$data['organization']));
		}
